...
class PredictionCallback(keras.callbacks.Callback):    
  def on_epoch_end(self, epoch, logs={}):
    #x, y = self.test_data
    print('epoch ',epoch+1)
    global x_train, y_train, x_val, y_val, x1_test, ans, validation_acc, test_acc
    loss1, acc1 = self.model.evaluate(x_val, y_val, verbose=0)
    print('\nvalidation loss: {}, acc: {}\n'.format(loss1, acc1))
    validation_acc.append(acc1)
    y_pred = self.model.predict(x1_test)
...
# tmp2 = tokenizer.fit_on_texts(test_df['text'])
...
x_train, x_val, y_train, y_val = train_test_split(x1_train, y1_train, test_size = VALIDATION_RATIO, random_state=1)
...
for dim in [128,256]:
    for batch in [8,16,32]:
        ...
        model = keras.models.Model(inputs = top_input, outputs = predictions)
        ...
        validation_acc = []
        test_acc = []
        history = model.fit( x=x_train, y=y_train, epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, validation_data=(x_val, y_val),callbacks=[PredictionCallback()], shuffle=True)
